...
import { pool, OPENROUTER_KEY } from '../config';
...
class AIService {
  async build(data: { prompt: string }, user: { id: number }): Promise<any> {
    ...
    try {
      ...
      logger.warn({ msg: 'AI build: falling back to env settings', error: e.message });
    }
      ...
      try {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          if (attempt > 0) {
            ...
            logger.info({ msg: `AI retry ${attempt}/3 for ${mdl}`, delay });
          ...
          if (response.status === 429) {
            ...
            logger.warn({ msg: `AI 429 on ${mdl}`, waitMs });
          ...
          if (!response.ok) {
            ...
            logger.error({ msg: `AI error (${mdl})`, status: response.status, errText });
          ...
          const spec = JSON.parse(jsonMatch[0]) as { title?: string; content?: any[] };
          if (!spec.title) spec.title = userPrompt.slice(0, 60);
          ...
          logger.info({ msg: `AI build success`, model: mdl });
          return spec;
        }
      } catch (e: any) {
        logger.error({ msg: `AI build error on ${mdl}`, error: e.message });
        lastError = e.message;
      }
    }

    logger.error({ msg: 'All AI models failed', lastError });
  ...
  async updateSettings(data: { userId: number; provider: string; apiKey?: string; model: string; temperature: number; maxTokens: number }): Promise<any> {
  ...
  async testConnection(userId: number): Promise<any> {
    ...
    const data = await testResponse.json() as { model?: string };
    return { success: true, message: 'Connection successful', model: data.model || model };
  }
}

export { AIService, AI_PROVIDER_MODELS };