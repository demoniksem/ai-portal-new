...
class SearchService {
  async search(opts: SearchParams): Promise<SearchResult | { error: string; status: number }> {
    if (!opts.query) {
    ...
    for (const indexName of indexes) {
      ...
      if (opts.spaceId && indexName === 'pages') filters.push(`spaceId = ${parseInt(opts.spaceId, 10)}`);
      if (opts.boardId && indexName === 'cards') filters.push(`board_id = '${opts.boardId}'`);
      if (filters.length > 0) searchParams.filter = filters.join(' AND ');

      try {
        const results = await meiliClient.index(indexName).search(opts.query, searchParams) as any;
        ...
        if (highlight && results.hits) {
          for (const hit of results.hits) {
            const entry: Record<string, string> = { id: hit.id, type: indexName };
            if (hit._formatted) {
              entry.title = hit._formatted.title ?? hit.title ?? '';
              if (indexName === 'pages') entry.content = hit._formatted.content ?? '';
              if (indexName === 'cards') entry.description = hit._formatted.description ?? '';
            } else {
              entry.title = hit.title ?? '';
              if (indexName === 'pages') entry.content = typeof hit.content === 'string' ? hit.content.substring(0, 200) : '';
              if (indexName === 'cards') entry.description = typeof hit.description === 'string' ? hit.description.substring(0, 200) : '';
            }
  ...
  async autocomplete(query: string, limit: number = 5): Promise<{ hits: { id: number | string; title: string; type: string }[] }> {
    if (!query) return { hits: [] };

    const results: { id: number | string; title: string; type: string }[] = [];

    for (const indexName of ['pages', 'cards']) {
      try {
        const res = await meiliClient.index(indexName).search(query, {
          limit,
