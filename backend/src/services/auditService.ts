...
class AuditService {
  ...
  async log(params: AuditLogParams): Promise<void> {
    try {
      await pool.query(
        `INSERT INTO audit_log (company_id, actor_id, action, object_type, object_id, old_value, new_value, metadata)
   ...
   */
  async logUserRegister(companyId: string, actorId: string, newUser: { id: string; email: string }): Promise<void> {
    await this.log({
      companyId,
      actorId,
   ...
   */
  async logUserLogin(companyId: string, actorId: string): Promise<void> {
    await this.log({
      companyId,
      actorId,
      action: 'user.login',
   ...
   */
  async logUserLogout(companyId: string, actorId: string): Promise<void> {
    await this.log({
      companyId,
      actorId,
      action: 'user.logout',
  ...
  async logRoleChange(
    ...
    oldRole: string | null,
    newRole: string,
    scope: 'company' | 'department' | 'object',
    scopeId?: string
  ): Promise<void> {
    await this.log({
      companyId,
  ...
  async query(params: {
    ...
    const result = await pool.query(
      `SELECT id, company_id, actor_id, action, object_type, object_id,
