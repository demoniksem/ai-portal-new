-- Migration: 002_cards
-- Kanban boards, cards, checklists, custom fields, comments, relations, activity log

DO $$ BEGIN
  CREATE TYPE card_type AS ENUM ('task', 'bug', 'story', 'epic');
...
DO $$ BEGIN
  CREATE TYPE relation_type AS ENUM ('blocks', 'blocked_by', 'duplicates', 'relates_to', 'parent', 'child');
EXCEPTION WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE IF NOT EXISTS boards (
  ...
  space_id UUID NOT NULL, -- references spaces(id) on legacy schema
...
CREATE TABLE IF NOT EXISTS custom_field_definitions (
  ...
  board_id UUID NOT NULL REFERENCES boards(id) ON DELETE CASCADE,
  name VARCHAR(100) NOT NULL,
  field_type VARCHAR(20) NOT NULL DEFAULT 'text', -- text|number|date|select|checkbox
  options JSONB DEFAULT '[]', -- for select type: [{value, label}]
  position INTEGER NOT NULL DEFAULT 0,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS card_custom_fields (
  ...
  value JSONB, -- flexible: string, number, boolean, array
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS card_comments (
  ...
  mentions JSONB DEFAULT '[]', -- [{userId, offset, length}]
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Card relations (many-to-many via junction)
...
CREATE TABLE IF NOT EXISTS card_templates (
  ...
  fields JSONB DEFAULT '{}', -- {priority, labelIds, checklistItems, ...}
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Indexes for cards module
