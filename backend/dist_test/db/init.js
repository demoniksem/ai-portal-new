...
async function initializeDatabase() {
    try {
        ...
        for (const stmt of enumStatements) {
            try {
                await config_1.pool.query(stmt);
            }
            catch (_) { /* ignore */ }
        }
        // Cards module ENUMs
    ...
    `);
        ...
        try {
            await config_1.pool.query('CREATE INDEX IF NOT EXISTS idx_rbac_users_deleted_at ON rbac_users(deleted_at) WHERE deleted_at IS NULL');
        }
        catch (_) { /* ignore */ }
        ...
        const existingAdmin = await config_1.pool.query('SELECT id FROM users WHERE email = $1', [adminEmail]);
        if (existingAdmin.rows.length === 0) {
            const hashed = await bcrypt.hash(adminPass, 10);
            await config_1.pool.query('INSERT INTO users (email, password_hash, username) VALUES ($1, $2, $3)', [adminEmail, hashed, 'admin']);
            logger_1.logger.info({ msg: 'Default admin created', email: 'admin@portal.com' });
        }
        ...
        if (process.env.MEILISEARCH_URL) {
            try {
                ...
                const meili = config_1.meiliClient;
                ...
                for (const cfg of indexConfigs) {
                    ...
                    await idx.updateFilterableAttributes(cfg.filterAttrs);
                    logger_1.logger.info({ msg: `MeiliSearch index ${cfg.uid} created/configured` });
                }
            }
            catch (e) {
                logger_1.logger.warn({ msg: 'MeiliSearch index setup note', error: e.message });
            }
        }
    }
    catch (e) {
        logger_1.logger.error({ msg: 'DB init error', error: e.message });
    }
}
